...
const Hero = ({ skipCurtains = false }) => {
  const { t } = useTranslation();

  // When skipCurtains=true (loader handled the reveal), shorten animation delays
  const base = skipCurtains ? 0.15 : 1.1;

  const fadeUp = (delay = 0) => ({
    initial: { opacity: 0, y: 30 },
    animate: { opacity: 1, y: 0, transition: { duration: 0.9, ease: [0.22, 1, 0.36, 1], delay: base + delay } },
  });
  const fadeIn = (delay = 0) => ({
    initial: { opacity: 0 },
    animate: { opacity: 1, transition: { duration: 1.1, ease: "easeOut", delay: base + delay } },
  });
    ...
    <>
      ...
      <section className="relative flex sm:flex-row flex-col w-full h-screen mx-auto sm:bg-hero bg-hero-mobile overflow-hidden">
        <div className={`absolute inset-0 sm:top-[250px] top-[150px] lg:top-[150px] xl:top-[250px] ${styles.paddingX} max-w-7xl mx-auto flex flex-row items-start justify-between gap-3`}>
          ...
          <div className="flex flex-col items-start justify-center sm:-ml-[3rem] xxs:mt-4" />
          <div />
        </div>
        ...
        <motion.div className="absolute xs:bottom-10 bottom-32 w-full flex justify-center items-center" variants={fadeIn(0.6)} initial="initial" animate="animate">
          <a href="#about">
            <div className="w-[35px] h-[64px] rounded-3xl border-4 border-french flex justify-center items-start p-2">
